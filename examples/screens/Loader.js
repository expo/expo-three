...
async function loadX() {
  const object = await ExpoTHREE.loadAsync(Assets.models.xfile["SSR06_Born2.x"], onProgress, name => Assets.models.xfile.texture[name]);
...
async function load3DS() {
  ...
  const object = await ExpoTHREE.loadAsync(model["portalgun.3ds"], onProgress, name => model.textures[name]);
