...
async function loadAssimp() {
  ...
  const object = await ExpoGame.loadAsync(res['Octaminator.assimp'], onProgress, name => res[name]);
  return object;
}
...
async function loadX() {
  ...
  const object = await ExpoGame.loadAsync(res['SSR06_Born2.x'], onProgress, name => res[name]);
...
async function loadBasicDAE() {
  ...
  const collada = await ExpoGame.loadAsync(model['elf.dae'], onProgress, name => model[name]);
...
async function loadRiggedDAE() {
  ...
  const collada = await ExpoGame.loadAsync(
...
async function load3DS() {
  ...
  const object = await ExpoGame.loadAsync(tds.portalgun, onProgress, name => tds.textures[name]);
...
async function loadObjImages() {
  ...
  const normalMap = await ExpoGame.loadAsync(obj['normal.jpg']);
  const aoMap = await ExpoGame.loadAsync(obj['ao.jpg']);
  const displacementMap = await ExpoGame.loadAsync(obj['displacement.jpg']);

  const object = await ExpoGame.loadAsync(obj['ninjaHead_Low.obj']);
...
async function load3MF() {
  const object = await ExpoGame.loadAsync(require('./models/3mf/cube_gears.3mf'), onProgress);
...
async function loadAMF() {
  const object = await ExpoGame.loadAsync(require('./models/amf/rook.amf'), onProgress);
...
async function loadBabylon() {
  ...
  const scene = await ExpoGame.loadAsync(require('./models/babylon/skull.babylon'), onProgress);
...
async function loadDraco() {
  ...
  const geometry = await ExpoGame.loadAsync(require('./models/draco/bunny.drc'), onProgress);
...
async function loadSTL(key = 'binary') {
  ...
  const geometry = await ExpoGame.loadAsync(models[key], onProgress);
...
async function loadPCD(key = 'binary') {
  ...
  const models = { binary, ascii };
  const mesh = await ExpoGame.loadAsync(models[key], onProgress);
...
async function loadMSGPack() {
  ...
  const scene = await ExpoGame.loadAsync(res, onProgress);
  return scene;
}
...
async function loadBVH() {
  ...
  const { skeleton, clip } = await ExpoGame.loadAsync(res, onProgress);
...
class Scene extends React.Component {
  ...
  onRender = delta => {
    ...
    option.onRender && option.onRender({ scene, renderer, mixer, camera, delta });
