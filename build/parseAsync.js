...
async function parseBinAsync({ json, onProgress, assetProvider }) {
    ...
    if (!buffers) {
        throw new Error(`ExpoTHREE.parseAsync: Invalid json! The json file must contain a "buffers" key.`);
    }
    const bin = await assetProvider(buffers);
    if (bin === undefined) {
        throw new Error(`ExpoTHREE.parseAsync: Cannot parse undefined .bin. Using the assetProvider you must pass a valid reference for: ${buffers}.`);
    }
...
export default async function parseAsync({ json, format, onProgress, assetProvider, }) {
