...
export default (loadAsync = async (res, onProgress, assetProvider) => {
  ...
  if (urls[0].match(/\.stl$/i) && urls.length === 1) {
    ...
    this.loader.setTexturePath(assetProvider);
    const file = await loadJsonFile(urls[0]);
    const model = this.loader.parse(file, onProgress);
    return model;
    ...
    const file = await loadRawFileAsync(urls[0]);
...
const loadRawFileAsync = async (localUri: string) => {
