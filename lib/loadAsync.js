...
export default (loadAsync = async (res, onProgress, assetProvider) => {
  ...
  if (urls[0].match(/\.stl$/i) && urls.length === 1) {
    ...
    return new Promise((res, rej) =>
      this.loader.load(urls[0], res, onProgress, rej),
    );
  } else if (
    urls.length === 1 &&
    (urls[0].match(/\.png$/i) || urls[0].match(/\.jpg$/i))
  ) {
    const { asset } = this;
    ...
    (urls[0].match(/\.vtk$/i) || urls[0].match(/\.vtp$/i)) &&
    urls.length === 1
  ) {
    ...
    return new Promise((res, rej) =>
      this.loader.load([urls[0], false], res, onProgress, rej),
    );
    ...
    const file = await Expo.FileSystem.readAsStringAsync(urls[0]);
    return this.loader.parse(file);
  } else if (
    urls.length === 2 &&
    urls[0].match(/\.mtl$/i) &&
    urls[1].match(/\.obj$/i)
  ) {
    return loadOBJMTL(urls[1], urls[0], onProgress, assetProvider);
  } else if (
    urls.length === 2 &&
    urls[0].match(/\.obj$/i) &&
    urls[1].match(/\.mtl$/i)
  ) {
    return loadOBJMTL(urls[0], urls[1], onProgress, assetProvider);
  } else {
    console.error('Unrecognized File Type', urls[0].split('.').pop(), urls[0]);
  }
});

const loadOBJMTL = async (obj, mtl, onProgress, assetProvider) => {
  const materials = await loadAsync(mtl, onProgress, assetProvider);
  materials.preload();
  ...
  const file = await Expo.FileSystem.readAsStringAsync(obj);
