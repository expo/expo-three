...
*/
export function suppressWarnings(shouldSuppress = true) {
  if (shouldSuppress) {
...
//-> [FeatureHitTestResult]
export function hitTestWithPoint(camera: THREE.Camera, point: Point): Array<any> {
  var results = [];
  const ray = hitTestRayFromScreenPos(camera, point);
  if (!ray) {
    return results;
...
export function unprojectPoint(camera: THREE.Camera, point: THREE.Vector3): THREE.Vector3 {
...
export function hitTestRayFromScreenPos(camera: THREE.Camera, point: Point) {
...
function _getRawFeaturePoints(rawFeaturePoints) {
  ...
  if (featurePoints == null) {
    const currentFrame = AR.getCurrentFrame({ rawFeaturePoints: true }) || {};
