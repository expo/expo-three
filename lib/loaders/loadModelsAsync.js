...
export async function loadTextureAsync({ asset }) {
  if (!asset) {
    throw new Error('ExpoTHREE.loadTextureAsync(): Cannot parse null asset')
      .return;
  }
  let nextAsset = asset;
  ...
  if (Platform.OS === 'web') {
    let nextRes = nextAsset;
    if (
      typeof nextAsset === 'object' &&
      nextAsset !== null &&
      nextAsset.downloadAsync
    ) {
      nextRes = nextAsset.localUri || nextAsset.uri;
    }
