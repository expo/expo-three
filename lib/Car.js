...
export default function() {
  ...
  this.loadParts = async function(bodyJson, wheelJson, assetProvider) {
    const body = await ExpoTHREE.parseAsync({
      format: 'bin',
      json: bodyJson,
      assetProvider,
    });
    const wheel = await ExpoTHREE.parseAsync({
      format: 'bin',
      json: wheelJson,
      assetProvider,
    });
  ...
  this.updateCarModel = function(delta) {
    ...
    if (!(forward || backward)) {
      if (this.speed > 0) {
        ...
        this.acceleration = THREE.Math.clamp(
          this.acceleration - k * delta,
          0,
          1,
        );
      } else {
    ...
    this.carOrientation +=
      forwardDelta * this.STEERING_RADIUS_RATIO * this.wheelOrientation;
  ...
  const createCar = ({ body, wheel }) => {
    ...
    this.frontLeftWheelMesh.scale.set(
      this.modelScale,
      this.modelScale,
      this.modelScale,
    );
    ...
    this.frontRightWheelMesh.scale.set(
      this.modelScale,
      this.modelScale,
      this.modelScale,
    );
    ...
    this.backLeftWheelMesh.scale.set(
      this.modelScale,
      this.modelScale,
      this.modelScale,
    );
    ...
    this.backRightWheelMesh.scale.set(
      this.modelScale,
      this.modelScale,
      this.modelScale,
    );
